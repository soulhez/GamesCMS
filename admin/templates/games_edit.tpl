{include file="header.tpl" title='Edit Game'}
<div class="container">
<div class="row">
<div class="page-header">
<h4>Edit Game {$game.name|stripslashes}<a href="{$settings['siteurl']}/{$settings['adminfolder']}/games_edit.php?type=delete&id={$game.id}" type="button" class="btn btn-danger" style="float:right" onclick="return confirm('Are you sure?')">Delete</a></h4>
</div>

{if $errors}<p class="bg-danger">{$errors}</p>{/if}
{if $msg && $gamelink}<p class="bg-success">{$msg}<br>Play it: <a href="{$settings['siteurl']}/en/g/{$gamelink}">{$game_name}</a></p>{else}
<form method="post" action="{$settings['siteurl']}/{$settings['adminfolder']}/games_edit.php?type=edit&id={$game.id}&submt=true" id="loginit" enctype="multipart/form-data">
...
<textarea name="embed" cols="50" rows="3" required id="exampleInputEmail8">{$game.swf}</textarea>
</div>

<div class="form-group" style="float:left;clear:both">
<label class="checkbox-inline">
  <input type="checkbox" id="inlineCheckbox1" name="feature" value="1" {if $game.feature==1}checked{/if}> Feature
</label>
<label class="checkbox-inline">
  <input type="checkbox" id="inlineCheckbox2" name="hasscores" value="1" {if $game.has_scores==1}checked{/if}> Scores
</label>
<label class="checkbox-inline">
  <input type="checkbox" id="inlineCheckbox3" name="hasawards" value="1" {if $game.has_achs==1}checked{/if}> Awards
</label>
<label class="checkbox-inline">
  <input type="checkbox" id="inlineCheckbox1" name="ads" value="1" {if $game.ads==1}checked{/if}> Ads
</label>
</div>

<div id="highscorediv" class="form-group" style="float:left;clear:both;{if $game.has_scores==0}display:none{/if}">
...
<div id="awardsdiv" class="form-group" style="float:left;clear:both;{if $game.has_achs==0}display:none{/if}">
<label>Awards</label>
{if $game.has_achs==1}
