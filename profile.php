...
try {
$stmt = $db->prepare("SELECT 1 + (SELECT count( * ) FROM users a WHERE a.xp > b.xp ) AS rank,id FROM users b WHERE id=:id ORDER BY rank LIMIT 1");
$stmt->bindValue(':id', $profile_uid, PDO::PARAM_INT);
$stmt->execute();
$results = $stmt->fetchAll(PDO::FETCH_ASSOC);
$userrank=$results[0];
$smarty->assign('userrank',$userrank['rank'],true);
} catch(PDOException $ex) {
if($smarty->debugging){
echo $ex->getMessage();
}
...
if($friends_count>0){
try {
$stmt = $db->prepare("SELECT GROUP_CONCAT(DISTINCT following_uid SEPARATOR ',') AS id_list FROM users_friends WHERE follower_uid=:id GROUP BY follower_uid LIMIT 1");
$stmt->bindValue(':id', $profile_uid, PDO::PARAM_INT);
$stmt->execute();
$results = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
$fids=$profile_friends['id_list'];

try {
$stmt = $db->query("SELECT 1 + (SELECT count( * ) FROM users a WHERE a.xp > b.xp AND a.id IN (".$fids.")) AS rank,id FROM users b WHERE id=$profile_uid ORDER BY rank LIMIT 1");
$stmt->bindValue(':id', $profile_uid, PDO::PARAM_INT);
$stmt->execute();
$results = $stmt->fetchAll(PDO::FETCH_ASSOC);
$friend_rank=$results[0];
} catch(PDOException $ex) {
if($smarty->debugging){
echo $ex->getMessage();
}
...
$friend_rank=$friend_rank['rank'];	
}else{
$friend_rank=1;
}
$smarty->assign('friend_rank',$friend_rank,true);
if($award_count>0){
try {
$stmt = $db->prepare("SELECT a.id,a.points,a.name,a.textdesc,g.url_key,g.name as gname,game.thumb FROM achievements a, users_achievemet ua,{$gamesdb} g,games game WHERE ua.uid=:id AND ua.aid=a.ach_id AND game.id=a.gid AND g.id=a.gid ORDER BY ua.id DESC LIMIT 5");
$stmt->bindValue(':id', $profile_uid, PDO::PARAM_INT);
$stmt->execute();
